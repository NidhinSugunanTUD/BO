function [cost, radcost] = runMPCnoise(weights, param, dim, model, realmodel, noise, do_plot)
if nargin < 7
    do_plot = 0;
end
...
for k = 1:dim.Nsim
    ...
    xk = sim.x(:, k);
    xknoise = xk  + noise(:, k);
    bnds.p   = [xknoise; xref]; % parameters
...
% cost = log(mean(0.1*abs(xref(1)-sim.x(1,:)) + 0.1*abs(xref(4)-sim.x(4,:))) + 1);
radcost = mean(0.1*abs(xref(1)-sim.x(1,:)) + 0.1*abs(xref(4)-sim.x(4,:)));
